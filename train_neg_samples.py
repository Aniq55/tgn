...
parser = argparse.ArgumentParser('TGN self-supervised training (explicit negatives)')
parser.add_argument('-d', '--data', type=str, help='Dataset name (eg. wikipedia or reddit)',
                    default='wikipedia')
parser.add_argument('--all_samples_data', type=str,
                    help='Dataset name for the ALL-SAMPLES superset (e.g. PHI_ctig_0). If empty, defaults to PHI_<data>.',
                    default='')
parser.add_argument('--other_data', type=str, help='Optional second dataset for testing (eg. ctig_dagger). If empty, no second-dataset eval is done.',
                    default='')
parser.add_argument('--other_all_samples_data', type=str,
                    help='ALL-SAMPLES dataset name for other_data. If empty and other_data is set, defaults to PHI_<other_data>.',
                    default='')
...
ALL_SAMPLES_DATA = args.all_samples_data.strip() if args.all_samples_data.strip() else f"PHI_{DATA}"
OTHER_ALL_SAMPLES_DATA = args.other_all_samples_data.strip() if args.other_all_samples_data.strip() else (f"PHI_{OTHER_DATA}" if OTHER_DATA else "")
...
### Extract data for training, validation and testing for the primary dataset (positives + explicit negatives)
(node_features, edge_features,
 full_data, train_data, val_data, test_data,
 new_node_val_data, new_node_test_data,
 neg_full_data, neg_train_data, neg_val_data, neg_test_data,
 neg_new_node_val_data, neg_new_node_test_data) = get_data(
    DATA, DISTORTION,
    different_new_nodes_between_val_and_test=args.different_new_nodes,
    randomize_features=args.randomize_features,
    all_samples_dataset_name=ALL_SAMPLES_DATA
)
...
other_neg_full_data = other_neg_train_data = other_neg_val_data = other_neg_test_data = None
other_neg_new_node_val_data = other_neg_new_node_test_data = None
other_full_ngh_finder = None
...
if OTHER_DATA:
    (other_node_features, other_edge_features,
     other_full_data, other_train_data, other_val_data, other_test_data,
     other_new_node_val_data, other_new_node_test_data,
     other_neg_full_data, other_neg_train_data, other_neg_val_data, other_neg_test_data,
     other_neg_new_node_val_data, other_neg_new_node_test_data) = get_data(
        OTHER_DATA,
        DISTORTION,
        split_time=t_value,
        different_new_nodes_between_val_and_test=args.different_new_nodes,
        randomize_features=args.randomize_features,
        all_samples_dataset_name=OTHER_ALL_SAMPLES_DATA
    )

# Initialize neighbor finders for primary dataset (POSITIVE graph only)
# IMPORTANT: explicit negatives may contain nodes that do not appear in train_data (or even full_data),
# so we size the neighbor finder adjacency lists to cover ALL node ids we might query.
def _safe_max_node_id(*datas):
    max_id = 0
    for d in datas:
        if d is None or len(d.sources) == 0:
            continue
        max_id = max(max_id, int(np.max(d.sources)), int(np.max(d.destinations)))
    return max_id

primary_max_node_id = _safe_max_node_id(full_data, neg_full_data, new_node_val_data, new_node_test_data,
                                        neg_new_node_val_data, neg_new_node_test_data)
train_ngh_finder = get_neighbor_finder(train_data, args.uniform, max_node_idx=primary_max_node_id)
full_ngh_finder = get_neighbor_finder(full_data, args.uniform, max_node_idx=primary_max_node_id)

# If other dataset provided, initialize its neighbor finder(s) with a safe max node id too
if OTHER_DATA and other_full_data is not None:
    other_max_node_id = _safe_max_node_id(other_full_data, other_neg_full_data,
                                          other_new_node_val_data, other_new_node_test_data,
                                          other_neg_new_node_val_data, other_neg_new_node_test_data)
    other_full_ngh_finder = get_neighbor_finder(other_full_data, args.uniform, max_node_idx=other_max_node_id)
...
# Compute time statistics (primary dataset, POSITIVES only)
...
def _edge_scores_update_memory(model, sources, destinations, timestamps, edge_idxs, n_neighbors):
  ...
  # affinity module name varies; try common ones
  affinity = None
  for name in ["affinity_score", "merge_layer", "decoder", "edge_predictor", "link_predictor"]:
    if hasattr(model, name):
      affinity = getattr(model, name)
      break
  if affinity is None:
    raise AttributeError("Model has no affinity/decoder module (expected affinity_score/merge_layer/decoder/edge_predictor/link_predictor).")
  return affinity(src_emb, dst_emb).sigmoid()


def _edge_scores_no_side_effects(model, sources, destinations, timestamps, edge_idxs, n_neighbors):
  if USE_MEMORY:
    mem_backup = model.memory.backup_memory()
  try:
    prob = _edge_scores_update_memory(model, sources, destinations, timestamps, edge_idxs, n_neighbors)
  finally:
    if USE_MEMORY:
      model.memory.restore_memory(mem_backup)
  return prob
...
for i in range(args.n_runs):
  ...
  # Positive train batches
  pos_num = len(train_data.sources)
  pos_num_batch = math.ceil(pos_num / BATCH_SIZE) if pos_num > 0 else 0

  logger.info('num of training POS instances: {}'.format(pos_num))
  logger.info('num of POS batches per epoch: {}'.format(pos_num_batch))
  logger.info('num of training NEG instances: {}'.format(len(neg_train_data.sources)))
  ...
  for epoch in range(NUM_EPOCH):
    ...
    # Train using only training graph (positives)
    ...
    neg_ptr = 0
    neg_n = len(neg_train_data.sources)

    for k in range(0, pos_num_batch, args.backprop_every):
      loss = 0.0
      optimizer.zero_grad()

      for j in range(args.backprop_every):
        batch_idx = k + j
        if batch_idx >= pos_num_batch:
          continue

        start_idx = batch_idx * BATCH_SIZE
        end_idx = min(pos_num, start_idx + BATCH_SIZE)

        # --- Positive batch (updates memory) ---
        ...
        if len(sources_batch) > 0:
          pos_label = torch.ones(len(sources_batch), dtype=torch.float, device=device)
          tgn = tgn.train()
          pos_prob = _edge_scores_update_memory(tgn, sources_batch, destinations_batch, timestamps_batch, edge_idxs_batch, NUM_NEIGHBORS)
          loss = loss + criterion(pos_prob.squeeze(), pos_label)

          # --- Consume ALL negatives with timestamp <= max timestamp in this positive batch ---
          t_horizon = float(np.max(timestamps_batch))
          while neg_ptr < neg_n and float(neg_train_data.timestamps[neg_ptr]) <= t_horizon:
            neg_end = min(neg_n, neg_ptr + BATCH_SIZE)
            while neg_end > neg_ptr and float(neg_train_data.timestamps[neg_end - 1]) > t_horizon:
              neg_end -= 1
            if neg_end == neg_ptr:
              break

            n_src = neg_train_data.sources[neg_ptr:neg_end]
            n_dst = neg_train_data.destinations[neg_ptr:neg_end]
            n_ts = neg_train_data.timestamps[neg_ptr:neg_end]
            n_ei = neg_train_data.edge_idxs[neg_ptr:neg_end]

            neg_label = torch.zeros(len(n_src), dtype=torch.float, device=device)

            # Score negatives WITHOUT permanently updating memory
            neg_prob = _edge_scores_no_side_effects(tgn, n_src, n_dst, n_ts, n_ei, NUM_NEIGHBORS)
            loss = loss + criterion(neg_prob.squeeze(), neg_label)
      ...
      loss.backward()
      optimizer.step()
      m_loss.append(float(loss.item()))

      # Detach memory so we don't backprop to the start of time
      if USE_MEMORY:
        tgn.memory.detach_memory()

    # After all positive batches, process any remaining negatives (still no memory update)
    while neg_ptr < neg_n:
      neg_end = min(neg_n, neg_ptr + BATCH_SIZE)
      n_src = neg_train_data.sources[neg_ptr:neg_end]
      n_dst = neg_train_data.destinations[neg_ptr:neg_end]
      ...
      tgn = tgn.train()
      neg_prob = _edge_scores_no_side_effects(tgn, n_src, n_dst, n_ts, n_ei, NUM_NEIGHBORS)
      neg_loss = criterion(neg_prob.squeeze(), neg_label)
      neg_loss.backward()
      optimizer.step()
      m_loss.append(float(neg_loss.item()))
    ...
    ### Validation (primary) with explicit negatives
    ...
    # Validate on unseen nodes (primary) with explicit negatives restricted to new-node edges
    _, nn_val_ap, nn_val_auc = eval_edge_prediction(model=tgn,
                                                    pos_data=new_node_val_data,
                                                    neg_data=neg_new_node_val_data,
                                                    n_neighbors=NUM_NEIGHBORS,
                                                    batch_size=BATCH_SIZE)
    ...
    train_losses.append(float(np.mean(m_loss)) if len(m_loss) else float("nan"))
    ...
    logger.info('Epoch mean loss: {}'.format(np.mean(m_loss) if len(m_loss) else float("nan")))
    logger.info('val auc: {}, new node val auc: {}'.format(val_auc, nn_val_auc))
    logger.info('val ap: {}, new node val ap: {}'.format(val_ap, nn_val_ap))

    # Early stopping
    if early_stopper.early_stop_check(val_ap if not np.isnan(val_ap) else -1.0):
  ...
  nn_test_acc, nn_test_ap, nn_test_auc = eval_edge_prediction(model=tgn,
                                                                          pos_data=new_node_test_data,
                                                                          neg_data=neg_new_node_test_data,
                                                                          n_neighbors=NUM_NEIGHBORS,
                                                                          batch_size=BATCH_SIZE)
  ...
  if OTHER_DATA and other_full_data is not None:
    ...
    other_test_acc, other_test_ap, other_test_auc = eval_edge_prediction(
        model=model_other,
        pos_data=other_test_data,
        neg_data=other_neg_test_data,
        n_neighbors=NUM_NEIGHBORS,
        batch_size=BATCH_SIZE
    )
    ...
    other_nn_test_acc, other_nn_test_ap, other_nn_test_auc = eval_edge_prediction(
        model=model_other,
        pos_data=other_new_node_test_data,
        neg_data=other_neg_new_node_test_data,
        n_neighbors=NUM_NEIGHBORS,
        batch_size=BATCH_SIZE
    )

    # CES imports / distances as in your original script
    ...
    info_combined = [MODEL_NAME, "Test", "tdv",
                     f"{bar_d_0_dagger:.4f}",
                     f"{acc_oracle:.4f}", f"{acc_dagger:.4f}",
                     f"{test_auc:.4f}", f"{other_test_auc:.4f}",
                     f"{test_ap:.4f}", f"{other_test_ap:.4f}"]
